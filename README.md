...
界面会用中文展示协作过程、发现的问题、影响范围、修复建议、验证结果和人工确认入口。工作台支持把 `.py` 或 Python 项目 `.zip` 拖入审查区，单个原始文件最大 100 MB；每次审查会保存到“审查历史”，可以随时重新打开。单文件可完成审查；想演示自动修复与测试验证时，请上传含 `tests/` 的 ZIP 项目包。
