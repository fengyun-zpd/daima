...
Dashboard 是 Frontend 交付物，先注册或登录，再按“选择代码 → 选择审查方式 → 创建并查看结果”操作；
顶部显示真实用户名、工号和账户权限。代码来源支持 unified diff、Python 项目 ZIP，以及一次选择或拖入多个本机 `.py` 文件。
浏览器在本地读取 `.py` 内容并分别转换为新增文件 diff，绝不提交本机绝对路径；上传区会显示文件总数和可展开的文件名清单。
默认的 **A2A 多 Agent 协作** 会让代码审查 Agent 与影响分析 Agent 分工处理，
界面会用中文展示协作过程、加载进度、发现的问题、影响范围、修复建议、验证结果和人工确认入口。所选原始文件总大小最大 100 MB；任务编号可自定义，不填则由系统生成。每次审查会保存到“审查历史”，可以随时重新打开。单文件可完成审查；想演示自动修复与测试验证时，请上传含 `tests/` 的 ZIP 项目包。
...
工作台在任务处理中每 2 秒自动更新一次结果，任务完成后自动停止；SSE 端点 `/api/v1/reviews/{task_id}/events` 亦可用于后续接入。这里的“自动更新”只是定时向系统询问最新进度，不会重新执行审查。
工作台登录后使用可撤销的 Bearer 令牌；密码以 PBKDF2 派生值保存，注册账户默认是 `developer` 权限。旧的 `X-Actor-Id` / `X-Actor-Role` 身份头仅为内部调用、自动化测试和已有 API 示例保留。写请求统一携带 `Idempotency-Key`，**同一次用户操作的重试复用同一个键**。
